...
negative, with two exceptions that require different interpretations: HIC carries
physicochemical signal, while Tm2 is partly predictable through framework/subtype
proxies.

![Internal GDPa1 cross-validation versus external GDPa3 held-out Spearman per assay; HIC survives as physicochemical signal while Tm2 is largely framework/subtype signal, with the published competition field ceiling marked.](results/headline_collapse.png)

*Per assay: internal sequence-cluster grouped cross-validation on GDPa1 versus a single prediction on the blinded GDPa3 held-out set. Most assays collapse externally. HIC holds as a physicochemical signal; raw Tm2 is also partially recoverable, but the best simple Tm2 signal is a framework/subtype proxy rather than a clean Fab-stability descriptor. Black bars mark the field ceiling — the best per-assay score across the 113 competition teams. Regenerate with `python scripts/plot_headline_collapse.py`.*
...
Tm2 is the second non-null endpoint, but it is not interpreted the same way as
HIC. A targeted Tm2 tier audit finds that raw Tm2 is best explained by
framework/subtype proxies (internal 0.29, external 0.33), close to the published
field ceiling of 0.39. After subtracting the GDPa1-estimated subtype expectation,
the residual Tm2 signal is weak. So Tm2 is predictable, but the result is better
...
performance. HIC is the clearest leakage-robust physicochemical signal. Tm2 is
partly predictable, but mainly through framework/subtype proxies; the remaining
assays collapse at the external step.
